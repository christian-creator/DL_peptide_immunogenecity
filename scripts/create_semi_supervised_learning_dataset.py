...
removed_idx = dataset_df[dataset_df.apply(lambda x: False if (x.HLA_allele) in hla_dic.keys() else True,axis=1)].index
print("These rows were removed due to HLA missing from the MHC_psuedo.dat: {}".format(removed_idx))
dataset_df = dataset_df[dataset_df.apply(lambda x: True if (x.HLA_allele) in hla_dic.keys() else False,axis=1)]
dataset_df.to_csv("../data/semi_supervised_data.csv")
# %%
